...
% --- CONTORNO ---
% Como agora é Argentina, o shape do Brasil não serve. 
% Se você não tiver o .shp da Argentina, use o 'landareas' do próprio MATLAB:
try
    world = shaperead('ar.shp', 'UseGeoCoords', true);
    for k = 1:length(world)
        plot(world(k).Lon, world(k).Lat, 'k', 'LineWidth', 1);
    end
catch
    disp('Não foi possível carregar o mapa base.');
end

% Limites para a Argentina
xlim([-75, -50])
ylim([-60, -25])

xlabel('Longitude')
ylabel('Latitude')
title('EOF1 - Argentina TEC 2025')
grid on;
axis equal;