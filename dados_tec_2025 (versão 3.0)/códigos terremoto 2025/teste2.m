...
% load('dados_workspace-terremoto.mat', 'score', 'coeff', 'latent', 'explained', 'tsquared', 'names');
 
% pca1 = score(:,1) * coeff(:,1)';

% pca1_plot = score(:,1);  % score(:,1) já é [tempo x 1] para a 1ª componente
% 
...
idx_num = idx_num(lia);  % remove estações não encontradas
...
for i = 1:length(idx_num)
    ...
    % Pega os dados PCA já calculados
    vtec_data = score(:,3) * coeff(:,3)';   % ou se você salvou pca1: vtec_data = pca1(:,ind);
    ...
    % Ticks principais e menores
    major_ticks = linspace(t_inicio, t_fim, 5);
    xticks(major_ticks);
    xticklabels(datestr(major_ticks,'HH:MM'));
    minor_ticks = linspace(t_inicio, t_fim, 17);
