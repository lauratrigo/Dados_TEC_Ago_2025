...
% Define major ticks
major_ticks = datenum(2025, 8, 22):datenum(2025, 8, 23); % Major ticks from October 12th to 23rd
...
for i = 1:length(files)
    ...
    time = linspace(datenum(2025,8,22), datenum(2025,8,23), length(vtec_data));
    ...
    % ==============================
    janela_longa = 60;   % 16 minutos
    media_longa = movmean(diff_TEC, janela_longa, 'omitnan');

    % Se quiser manter a banda tipo 2–10 mHz,
    % mantenha uma janela curta pequena
    janela_curta = 3;    % 3 minutos
    media_curta = movmean(diff_TEC, janela_curta, 'omitnan');

    % Residual tipo passa-banda aproximado
    dTEC = media_curta - media_longa;
    
    % ==============================
    % NORMALIZAÇÃO DO diff_TEC
    % ==============================
    diff_min = min(dTEC, [], 'omitnan');
    diff_max = max(dTEC, [], 'omitnan');

    %diff_TEC_norm = (dTEC - diff_min) ./ (diff_max - diff_min);
    
    matrix_diffTEC(:,i)=dTEC;
    plot(time, dTEC, 'b.-');
    ...
    for k = 1:length(diff_hF)
        if diff_hF(k,:) > diff_std(k,:)
            diff_hF(k,:) = diff_hF(k,:) - diff_std(k,:);
        elseif diff_hF(k,:) < -diff_std(k,:)
            diff_hF(k,:) = diff_hF(k,:) + diff_std(k,:);
        else
            diff_hF(k,:) = 0;    
        end
