...
for i = 1:length(files)
    ...
    % Leitura rápida: como você só quer contar NaNs, não precisamos de toda a lógica de vtec
    fid = fopen(files{i}, 'rt');
    % Lemos apenas as colunas de dados (ignorando o cabeçalho)
    raw_data = textscan(fid, '%s', 'Delimiter', '\n', 'HeaderLines', 1);
    fclose(fid);

    % Converter para numérico tratando vírgula e o código -999.0
    str_block = strrep(raw_data{1}, ',', '.');
    str_block = strrep(str_block, '-999.0', 'NaN');
    
    % Converte para matriz (cada linha do txt vira uma linha da matriz)
    data_mat = cell2mat(cellfun(@str2num, str_block, 'UniformOutput', false));
    
    % Nos seus arquivos, os dados de TEC geralmente começam na coluna 5 em diante
    vtec_focus = data_mat(:, 5:end);
    
    % Cálculo da porcentagem de NaN
    total_elementos = numel(vtec_focus);
    total_nan = sum(isnan(vtec_focus(:)));
    porcentagem_nan = (total_nan / total_elementos) * 100;
