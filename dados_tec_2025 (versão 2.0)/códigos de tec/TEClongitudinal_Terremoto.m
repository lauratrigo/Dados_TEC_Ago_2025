...
% stations = {'TOLH', 'SICO', 'GRCA', '25MA', 'SAIS', 'MZRF', 'LARJ', 'TAVA'};

stations = {'TAVA', 'LARJ', 'MZRF', 'SAIS', '25MA', 'GRCA', 'SICO', 'TOLH'};
...
for i = 1:length(idx_num)
    ...
    % Eixo do tempo
    % -------------------------
    % vetor de tempo distribuído ao longo de agosto
    time = linspace(datenum(2025,8,1), ...
                    datenum(2025,9,1), ...
                    length(vtec_data));
                
    % seleciona só o período de tempo desejado
    idx_time = time >= t_inicio & time <= t_fim; 
    
    time = time(idx_time);
    vtec_data = vtec_data(idx_time);
    ...
    major_ticks = datenum(2025, 8, 1):datenum(2025, 9, 1); 
