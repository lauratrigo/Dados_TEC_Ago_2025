...
% -------------------------
% REMO??O DE ESTA??ES EXTRAS (RS e Uruguai)
% -------------------------

idx_remove = ismember(names, {'3ARO', 'ABRA', 'ALUM', 'CSLO', 'DINO', 'ECAR', 'EPSF', 'GGUA', 'ITAI', 'MGNO', 'MZAU', 'MZGA', 'OJOA', 'RIO2', 'RSAL', 'RSAU', 'SANT', 'TUC1', 'UYSO'});
...
% -------------------------
n_pca = 1; % Escolha a PCA (Ex: PCA 1 geralmente ? o ciclo di?rio)
...
eof1 = coeff(:,n_pca:end); 
eof1=eof1/max(eof1);

% -------------------------
% CONFIGURA??O TEMPORAL (1 min)
% -------------------------
...
xlabel('Frequ?ncia (Ciclos por Minuto)')
xlim([0.001 1])
grid on;
...
% axis equal
% 
% title('Verifica??o da simetriza??o')
% legend('Pontos simetrizados','Esta??es reais')
...
for i = 1:length(shapefiles)
    try
        ...
        % Plota cada pol?gono do arquivo
        for k = 1:length(S)
            plot(S(k).Lon, S(k).Lat, 'k', 'LineWidth', 1);
        end
    catch
        fprintf('Aviso: N?o foi poss?vel carregar o arquivo %s\n', shapefiles{i});
    end
end

% Limites ajustados para abranger os novos pa?ses (ajuste se necess?rio)
xlim([-76, -50])
ylim([-60, -20])
...
% % Permite que voc? use o scroll do mouse para dar zoom e o clique para arrastar
% pan on; 
% zoom on;
% 
% % Garante que, ao dar zoom, a propor??o 1:1 se mantenha
