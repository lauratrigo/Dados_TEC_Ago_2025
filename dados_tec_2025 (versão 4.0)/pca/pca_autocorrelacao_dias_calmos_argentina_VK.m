...
% -------------------------
n_pca = 4; % Escolha a PCA (Ex: PCA 1 geralmente é o ciclo diário)
...
% --- PLOT DA ANÁLISE ---
figure('Color',[1 1 1], 'Name', sprintf('Temporal Analyses (1-min - PCA %d)', n_pca));

% Subplot 1: Score ao longo dos 31 dias
subplot(3,1,1);
plot(t_dias, score_analise, 'Color', [0 0.4470 0.7410]);
title(sprintf('Temporal PCA %d Variation (August 2025)', n_pca));
xlabel('Time (Days)');
...
title('Autocorrelation (Daily periodicity)');
xlabel('Delay (Hours)');
ylabel('Correlation');
...
title('Power Spectrum  (Dominant Frequencies)');
...
%title(sprintf('Geomagnetic Quiet Days - PCA %d', n_pca))
grid on;
% axis equal;
daspect([1 1 1]);

saveas(gcf, 'pca4_quiet_days.png');
