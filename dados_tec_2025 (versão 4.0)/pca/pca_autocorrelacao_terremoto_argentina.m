...
% -------------------------
% coordenadas das estações gnss [latitude longitude]
% -------------------------
coords = [
    -38.382, -60.274; % 3ARO
    -37.772, -67.716; % 25MA
...
lat = coords(:,1); % vetor das latitudes
lon = coords(:,2); % vetor das longitudes
...
idx_remove = ismember(names, {'25MA', 'ABRA', 'ALUM', 'CHAC', 'CHOY', 'CORD', 'CSLO', 'DINO', 'ECAR', 'EPGZ', 'GGUA', 'IGM1', 'JVCH', 'LROS', 'MGNO', 'MGUE', 'MZAU', 'MZGA', 'OJOA', 'RSAL', 'RSAU', 'SANT', 'SUAR', 'TUC1', 'UNSA', 'UYAR', 'UYPA', 'UYSO', 'YEMA'});

% remoção das coordenadas e nomes correspondentes
lat(idx_remove) = [];
lon(idx_remove) = [];
names(idx_remove) = [];
...
% -------------------------
n_pca = 1; % componente principal escolhida

matrix_list = readmatrix('matrix_diffTEC_Aug2025_terremoto.txt'); % importa o arquivo com os dados das estações

% matrix_list(isnan(matrix_list)) = 0; % substituí os valores faltantes por 0

% recorte temporal que remove o início e o final do mês
matrix_list(1:135,:) =[];
matrix_list(376:end,:) =[];

matrix_list(:, idx_remove) = []; % remove as colunas das estações excluídas

% [coeff,score,latent,tsquared,explained] = pca(matrix_list); % pca

[coeff,score,latent,tsquared,explained] = pca(matrix_list,'Algorithm','als','Centered',true); % pca com als

eof1 = coeff(:,n_pca); % seleciona a componente desejadas
eof1=eof1/max(eof1); % normaliza

% -------------------------
% ANÁLISE TEMPORAL DA PCA
% -------------------------
Fs = 1/60; % frequência de amostragem (1 dado por minuto)

score_analise = score(:, n_pca);

t_minutos = (0:length(score_analise)-1)'; % vetor do tempo em minutos

% autocorrelação (para identificar periodicidades)
max_lag = 60; % 60 minutos
[autocorr_vals, lags] = xcorr(score_analise - mean(score_analise), max_lag, 'coeff');
lags_min = lags;

% espectro (periodograma, mostra frequências dominantes)
[pxx, f] = periodogram(score_analise, rectwin(length(score_analise)), length(score_analise), Fs);

f_ciclos_minuto = f * 60; % converte Hz para ciclos por minuto

% plots da segunda digura
...
% contorno dos países (lista com todos os arquivos shapefile)
...
for i = 1:length(shapefiles)
    try
        ...
        % plote de cada polígono do arquivo
        for k = 1:length(S)
